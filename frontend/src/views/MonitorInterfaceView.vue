<template>
  <section class="surface monitor-page">
    <div class="monitor-toolbar">
      ...
      <el-button v-if="hasPermission('monitor.api.manage')" type="primary" @click="openForm()"><el-icon><Plus/></el-icon>新建接口</el-button>
    </div>
    <el-table v-loading="loading" :data="items" empty-text="暂无监控接口">
      ...
      <el-table-column label="状态" width="90"><template #default="{row}"><el-switch v-model="row.enabled" :disabled="!hasPermission('monitor.api.manage')" @change="toggle(row)"/></template></el-table-column>
      <el-table-column prop="created_by_name" label="创建人" width="120"/>
      <el-table-column label="创建时间" width="165"><template #default="{row}">{{formatTime(row.created_at)}}</template></el-table-column>
      <el-table-column label="操作" width="150" fixed="right"><template #default="{row}"><el-button v-if="hasPermission('monitor.api.manage')" link type="primary" @click="openForm(row)">编辑</el-button><el-button v-if="hasPermission('monitor.api.manage')" link type="danger" @click="remove(row)">删除</el-button></template></el-table-column>
    </el-table>
