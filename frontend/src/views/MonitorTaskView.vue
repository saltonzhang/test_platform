...
async function save(){if(!hasPermission('monitor.task.manage')||!await formRef.value?.validate().catch(()=>false))return;saving.value=true;try{const payload={name:form.name.trim(),api_type:form.api_type,environment:form.environment,interval_value:form.interval_value,interval_unit:form.interval_unit,enabled:form.enabled,notification:{enabled:false}};form.id?await api.updateMonitorTask(form.id,payload):await api.createMonitorTask(payload);dialog.value=false;await load();ElMessage.success('监控任务保存成功')}catch(e){ElMessage.error((e as Error).message)}finally{saving.value=false}}
...
async function runNow(item:MonitorTask){if(!hasPermission('monitor.task.run'))return;runningId.value=item.id;try{await api.runMonitorTask(item.id);await load();ElMessage.success('监控任务执行完成')}catch(e){ElMessage.error((e as Error).message)}finally{runningId.value=0}}
